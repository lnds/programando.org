...
def getResultOp(tupla):
    s =  '(%s %s %s)'%tuple(map(lambda t:prts[t],tupla))
    if opsresult.has_key(s):
        return opsresult.get(s)
    r = tupla[1](tupla[0], tupla[2])
    opsresult[s] = r
    return r
